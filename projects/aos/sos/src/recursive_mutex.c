...
int sync_recursive_mutex_new(sync_recursive_mutex_t *mutex)
{
    ut_t *ut = alloc_retype(&mutex->notification, seL4_NotificationObject, seL4_NotificationBits);

    if (!ut) {
        return -1;
    } else {
        mutex->notification_ut = ut;
        return sync_recursive_mutex_init(mutex, mutex->notification);
    }
}
...
{
    if (mutex == NULL) {
        ZF_LOGE("Recursive mutex passed to sync_recursive_mutex_destroy was NULL");
        return -1;
    }

    cspace_delete(&cspace, mutex->notification);
    cspace_free_slot(&cspace, mutex->notification);
    ut_free(mutex->notification_ut);
    return 0;
}
